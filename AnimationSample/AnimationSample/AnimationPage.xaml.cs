...
namespace AnimationSample
{
    public partial class AnimationPage : ContentPage
    {
        ...
        private void AniomateHeightB2_Clicked(object sender, EventArgs e)
        {
            ...
            if (b2Visible)
            {
                animation = new Animation(v => box2.HeightRequest = v, b2Height, 0, easing: Easing.SinOut);
                box2.Animate("b2", animation);
            }
            else
            {
                animation = new Animation(v => box2.HeightRequest = v, 0, b2Height, easing: Easing.SinIn);
                box2.Animate("b2", animation);
            }
